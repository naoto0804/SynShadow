...
class IterationCounter():
    def __init__(self, opt, dataset_size):
        ...
        self.first_epoch = 1
        if opt.niter > 0:
            self.total_epochs = opt.niter // dataset_size + 1
            self.opt.nepoch = self.total_epochs
            print(
                f"Automatically infer nepoch {opt.nepoch} from opt.niter and opt.batch_size")
            opt.nepoch_decay = 0
        else:
            self.total_epochs = opt.nepoch + opt.nepoch_decay

        self.epoch_iter = 0  # iter number within each epoch
        self.iter_record_path = Path(
    ...
    def record_one_iteration(self):
        ...
        self.time_per_iter = (
            current_time - self.last_iter_time) / self.opt.batch_size
